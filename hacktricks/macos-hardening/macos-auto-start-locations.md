...
This section is heavily based on the blog series [[https://theevilbit.github.io/beyond/), the goal is to add **more Autostart Locations** (if possible), indicate **which techniques are still working** nowadays with latest version of macOS (13.4|**Beyond the good ol' LaunchAgents**]] and to specify the **permissions** needed.
...
<dict>
    ...
    ProgramArguments
    <array>
        bash \-c 'touch /tmp/launched' <!--Prog to execute-->
    </array>
    RunAtLoad <!--Execute at system startup-->
    StartInterval
    800 <!--Execute each 800s-->
    KeepAlive
    <dict>
        SuccessfulExit</false> <!--Re-execute if exit unsuccessful-->
...
```
```
There are cases where an **agent needs to be executed before the user logins**, these are called **PreLoginAgents**. For example, this is useful to provide assistive technology at login. They can be found also in `/Library/LaunchAgents`(see [[https://github.com/HelmutJ/CocoaSampleCode/tree/master/PreLoginAgents) an example|**here**]].
...
<dict>
	CommandString
	mkdir /tmp/Documents; cp \-r \~/Documents /tmp/Documents;
...
If the popular program [[https://github.com/matryer/xbar|**xbar**]] is installed, it's possible to write a shell script in **`~/Library/Application\ Support/xbar/plugins/`** which will be executed when xbar is started:
...
[[https://github.com/Hammerspoon/hammerspoon|**Hammerspoon**]] serves as an automation platform for **macOS**, leveraging the **LUA scripting language** for its operations. Notably, it supports the integration of complete AppleScript code and the execution of shell scripts, enhancing its scripting capabilities significantly.
...
**Login items** can **also** be indicated in using the API [[https://developer.apple.com/documentation/servicemanagement/1501557-smloginitemsetenabled?language=objc|SMLoginItemSetEnabled]] which will store the configuration in **`/var/db/com.apple.xpc.launchd/loginitems.501.plist`**
...
1. Crafting a Folder Action workflow with [[https://support.apple.com/guide/automator/welcome/mac|Automator]] and installing it as a service.
...
**Compile a color picker** bundle with your code (you could use [[https://github.com/viktorstrate/color-picker-plus)) and add a constructor (like in the [Screen Saver section](macos-auto-start-locations.md#screen-saver)|**this one for example**]] and copy the bundle to `~/Library/ColorPickers`.
...
An application example with a Finder Sync Extension [[https://github.com/D00MFist/InSync|**can be found here**]].
...
```
> [!CAUTION]
> If you check the Plist of other `mdimporter` you might not find the entry **`UTTypeConformsTo`**. Thats because that is a built-in _Uniform Type Identifiers_ ([[https://en.wikipedia.org/wiki/Uniform_Type_Identifier)|UTI]] and it doesn't need to specify extensions.
>
...
Writeup: [[../linux-hardening/linux-post-exploitation/pam-pluggable-authentication-modules.md|Linux Hacktricks PAM]]\
...
```
```
For more information about [[macos-security-and-privilege-escalation/mac-os-architecture/README.md#I O Kit Drivers|kernel extensions check this section]].
