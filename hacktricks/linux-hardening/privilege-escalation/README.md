...
```
```
You can find a good vulnerable kernel list and some already **compiled exploits** here: [[https://github.com/lucyoa/kernel-exploits) and [exploitdb sploits](https://gitlab.com/exploit-database/exploitdb-bin-sploits|https://github.com/lucyoa/kernel-exploits]].\
Other sites where you can find some **compiled exploits**: [[https://github.com/bwbwbwbw/linux-exploit-binaries), [https://github.com/Kabot/Unix-Privilege-Escalation-Exploits-Pack](https://github.com/Kabot/Unix-Privilege-Escalation-Exploits-Pack|https://github.com/bwbwbwbw/linux-exploit-binaries]]
...
[[https://github.com/mzet-/linux-exploit-suggester|linux-exploit-suggester.sh]]\
[[https://github.com/jondonas/linux-exploit-suggester-2|linux-exploit-suggester2.pl]]\
[[http://www.securitysift.com/download/linuxprivchecker.py) (execute IN victim,only checks exploits for kernel 2.x|linuxprivchecker.py]]
...
```
```
Always check for possible [[electron-cef-chromium-debugger-abuse.md|**electron/cef/chromium debuggers** running, you could abuse it to escalate privileges]]. **Linpeas** detect those by checking the `--inspect` parameter inside the command line of the process.\
Also **check your privileges over the processes binaries**, maybe you can overwrite someone.

### Process monitoring

You can use tools like [[https://github.com/DominicBreuker/pspy|**pspy**]] to monitor processes. This can be very useful to identify vulnerable processes being executed frequently or when a set of requirements are met.
...
- [[https://github.com/hajzer/bash-memory-dump) (root|**https://github.com/hajzer/bash-memory-dump**]] - \_You can manually remove root requirements and dump the process owned by you
- Script A.5 from [[https://www.delaat.net/rp/2016-2017/p97/report.pdf) (root is required|**https://www.delaat.net/rp/2016-2017/p97/report.pdf**]]
...
#### Search Regexes/[[https://github.com/controlplaneio/truffleproc|truffleproc]]
...
```
```
**You can also use** [[https://github.com/DominicBreuker/pspy/releases) (this will monitor and list every process that starts|**pspy**]].
...
Note that if you have write permissions over the docker socket because you are **inside the group `docker`** you have [[interesting-groups-linux-pe/README.md#Docker Group|more ways to escalate privileges]]. If the [[../../network-services-pentesting/2375-pentesting-docker.md#compromising|**docker API is listening in a port** you can also be able to compromise it]].
...
Some Linux versions were affected by a bug that allows users with **UID > INT_MAX** to escalate privileges. More info: [[https://gitlab.freedesktop.org/polkit/polkit/issues/74), [here](https://github.com/mirchr/security-research/blob/master/vulnerabilities/CVE-2018-19788.sh) and [here](https://twitter.com/paragonsec/status/1071152249529884674|here]].\
...
If don't mind about doing a lot of noise and `su` and `timeout` binaries are present on the computer, you can try to brute-force user using [[https://github.com/carlospolop/su-bruteforce|su-bruteforce]].\
[[https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite|**Linpeas**]] with `-a` parameter also try to brute-force users.
...
[[payloads-to-execute.md|Payload examples to execute.]]
...
[[https://gtfobins.github.io) is a curated list of Unix binaries that can be exploited by an attacker to bypass local security restrictions. [**GTFOArgs**](https://gtfoargs.github.io/|**GTFOBins**]] is the same but for cases where you can **only inject arguments** in a command.
...
If you can access `sudo -l` you can use the tool [[https://github.com/CyberOne-Security/FallofSudo|**FallOfSudo**]] to check if it finds how to exploit any sudo rule.
...
If you have **write permissions** in the folder or on any of the created files inside the folder you can use the binary [[https://github.com/nongiach/sudo_inject/tree/master/extra_tools|**write_sudo_token**]] to **create a sudo token for a user and PID**.\
...
Note that if the user uses a different shell (not bash) you will need to modify other files to add the new path. For example[[https://github.com/APTy/sudo-piggyback| sudo-piggyback]] modifies `~/.bashrc`, `~/.zshrc`, `~/.bash_profile`. You can find another example in [[https://github.com/n00py/pOSt-eX/blob/master/empire_modules/bashdoor.py|bashdoor.py]]
...
Read the code of [[https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/linPEAS|**linPEAS**]], it searches for **several possible files that could contain passwords**.\
**Another interesting tool** that you can use to do so is: [[https://github.com/AlessandroZ/LaZagne|**LaZagne**]] which is an open source application used to retrieve lots of passwords stored on a local computer for Windows, Linux & Mac.
...
```
```
In order to **read logs the group** [[interesting-groups-linux-pe/README.md#Adm Group|adm]] will be really helpful.
...
I'm not going to list here how to do all of this but if you are interested you can check the last checks that [[https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/blob/master/linPEAS/linpeas.sh|**linpeas**]] perform.
...
You can exploit this vulnerability with [[https://github.com/whotwagner/logrotten|**logrotten**]].

This vulnerability is very similar to [[https://www.cvedetails.com/cve/CVE-2016-1247/) **(nginx logs|**CVE-2016-1247**]],** so whenever you find that you can alter logs, check who is managing those logs and check if you can escalate privileges substituting the logs by symlinks.
...
**LinEnum**: [[https://github.com/rebootuser/LinEnum)(-t option|https://github.com/rebootuser/LinEnum]]\
**Enumy**: [[https://github.com/luke-goddard/enumy|https://github.com/luke-goddard/enumy]]\
**Unix Privesc Check:** [[http://pentestmonkey.net/tools/audit/unix-privesc-check|http://pentestmonkey.net/tools/audit/unix-privesc-check]]\
**Linux Priv Checker:** [[http://www.securitysift.com/download/linuxprivchecker.py|www.securitysift.com/download/linuxprivchecker.py]]\
...
- [[https://0xdf.gitlab.io/2025/08/30/htb-eureka.html|0xdf – HTB Eureka (bash arithmetic injection via logs, overall chain)]]
- [[https://www.gnu.org/software/bash/manual/bash.html#index-BASH_005fENV|GNU Bash Manual – BASH_ENV (non-interactive startup file)]]
- [[https://0xdf.gitlab.io/2025/09/06/htb-environment.html|0xdf – HTB Environment (sudo env_keep BASH_ENV → root)]]
