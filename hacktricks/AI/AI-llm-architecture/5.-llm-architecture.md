...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layers[[x)# x shape: (batch_size, seq_len, 4 * emb_dim|0]]
        x = self.layers[[x) # x shape remains: (batch_size, seq_len, 4 * emb_dim|1]]
        x = self.layers[[x) # x shape: (batch_size, seq_len, emb_dim|2]]
