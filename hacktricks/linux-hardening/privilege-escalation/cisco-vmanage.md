...
After digging a little through some [[http://66.218.245.39/doc/html/rn03re18.html|documentation]] related to `confd` and the different binaries (accessible with an account on the Cisco website), we found that to authenticate the IPC socket, it uses a secret located in `/etc/confd/confd_ipc_secret`:
