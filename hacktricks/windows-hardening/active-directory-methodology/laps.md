...
You could **download the raw LAPS policy** from `\\dc\SysVol\domain\Policies\{4A8A4E8E-929F-401A-95BD-A7D40E0976C8}\Machine\Registry.pol` and then use **`Parse-PolFile`** from the [[https://github.com/PowerShell/GPRegistryPolicyParser|**GPRegistryPolicyParser**]] package can be used to convert this file into human-readable format.
...
The [[https://github.com/leoloobeek/LAPSToolkit|LAPSToolkit]] facilitates the enumeration of LAPS this with several functions.\
...
The original source code for LAPS can be found [[https://github.com/GreyCorbel/admpwd|here]], therefore it's possible to put a backdoor in the code (inside the `Get-AdmPwdPassword` method in `Main/AdmPwd.PS/Main.cs` for example) that will somehow **exfiltrate new passwords or store them somewhere**.
