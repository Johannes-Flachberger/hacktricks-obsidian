...
[[https://www.slideshare.net/i0n1c/syscan360-stefan-esser-os-x-el-capitan-sinking-the-ship|**Learn more about SIP info in this talk**]]**.**
...
#### [[https://objective-see.org/blog/blog_0x42.html|CVE-2019-8561]] 
...
#### [[https://objective-see.org/blog/blog_0x4D.html|CVE-2020–9854]] 
...
[[https://www.microsoft.com/en-us/security/blog/2021/10/28/microsoft-finds-new-macos-vulnerability-shrootless-that-could-bypass-system-integrity-protection/) discovered a vulnerability in macOS's System Integrity Protection (SIP|**Researchers from this blog post**]] mechanism, dubbed the 'Shrootless' vulnerability. This vulnerability centers around the **`system_installd`** daemon, which has an entitlement, **`com.apple.rootless.install.heritable`**, that allows any of its child processes to bypass SIP's file system restrictions.
...
#### [[https://perception-point.io/blog/technical-analysis-cve-2022-22583/|**CVE-2022-22583**]]

In [[https://perception-point.io/blog/technical-analysis-cve-2022-22583/|**CVE-2022-22583**]] it was discovered that the same **`system_installd`** process could still be abused because it was putting the **post-install script inside a random named folder protected by SIP inside `/tmp`**. The thing is that **`/tmp` itself isn't protected by SIP**, so it was possible to **mount** a **virtual image on it**, then the **installer** would put in there the **post-install script**, **unmount** the virtual image, **recreate** all the **folders** and **add** the **post installation** script with the **payload** to execute.

#### [[https://www.theregister.com/2016/03/30/apple_os_x_rootless/|fsck_cs utility]]
...
#### [[https://www.slideshare.net/i0n1c/syscan360-stefan-esser-os-x-el-capitan-sinking-the-ship|Mount over SIP protected folders]]
...
```
```
#### [[https://objective-see.org/blog/blog_0x14.html|Upgrader bypass (2016)]]
...
#### [[https://www.youtube.com/watch?v=zxZesAN-TEk|systemmigrationd (2023)]]

In this talk from [[https://www.youtube.com/watch?v=zxZesAN-TEk|**DEF CON 31**]], it's shown how **`systemmigrationd`** (which can bypass SIP) executes a **bash** and a **perl** script, which can be abused via env variables **`BASH_ENV`** and **`PERL5OPT`**.

#### CVE-2023-42860 

As [[https://blog.kandji.io/apple-mitigates-vulnerabilities-installer-scripts|**detailed in this blog post**]], a `postinstall` script from `InstallAssistant.pkg` packages allowed was executing:
...
The entitlement `com.apple.rootless.install` is known to bypass System Integrity Protection (SIP) on macOS. This was notably mentioned in relation to [[https://jhftss.github.io/CVE-2022-26712-The-POC-For-SIP-Bypass-Is-Even-Tweetable/|**CVE-2022-26712**]].
...
_+-- Container disk3 966B902E-EDBA-4775-B743-CF97A0556A13
|   ====================================================
