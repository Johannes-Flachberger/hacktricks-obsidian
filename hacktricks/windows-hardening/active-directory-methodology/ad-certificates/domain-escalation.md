...
You can use [[https://github.com/GhostPack/Certify) or [**Certipy**](https://github.com/ly4k/Certipy|**Certify**]] to abuse this scenario:
...
![[../../../images/image (814).png|]]


ESC4 is when a user has write privileges over a certificate template. This can for instance be abused to overwrite the configuration of the certificate template to make the template vulnerable to ESC1.

As we can see in the path above, only `JOHNPC` has these privileges, but our user `JOHN` has the new `AddKeyCredentialLink` edge to `JOHNPC`. Since this technique is related to certificates, I have implemented this attack as well, which is known as [[https://posts.specterops.io/shadow-credentials-abusing-key-trust-account-mapping-for-takeover-8ee1a53566ab|Shadow Credentials]]. Here’s a little sneak peak of Certipy’s `shadow auto` command to retrieve the NT hash of the victim.
...
The subject discussed in the [[https://cqureacademy.com/blog/enhanced-key-usage|**CQure Academy post**]] also touches on the **`EDITF_ATTRIBUTESUBJECTALTNAME2`** flag's implications, as outlined by Microsoft. This configuration, when activated on a Certification Authority (CA), permits the inclusion of **user-defined values** in the **subject alternative name** for **any request**, including those constructed from Active Directory®. Consequently, this provision allows an **intruder** to enroll through **any template** set up for domain **authentication**—specifically those open to **unprivileged** user enrollment, like the standard User template. As a result, a certificate can be secured, enabling the intruder to authenticate as a domain administrator or **any other active entity** within the domain.
...
```
```
Tools like [[https://github.com/GhostPack/Certify) and [**Certipy**](https://github.com/ly4k/Certipy|**Certify**]] are capable of detecting this misconfiguration and exploiting it:
...
[[https://github.com/GhostPack/Certify|**Certify**]]’s `cas` enumerates **enabled HTTP AD CS endpoints**:
...
```
```
#### Abuse with [[https://github.com/ly4k/Certipy|Certipy]]
...
A technique like [[https://github.com/ly4k/PetitPotam|PetitPotam]] can then be employed to coerce authentication. When dealing with domain controllers, the specification of `-template DomainController` is required.
...
If CA Server Do not configured with `IF_ENFORCEENCRYPTICERTREQUEST`, it can be makes NTLM relay attacks without signing via RPC service. [[https://blog.compass-security.com/2022/11/relaying-to-ad-certificate-services-over-rpc/|Reference in here]].
...
Or using [[https://github.com/sploutchy/impacket|sploutchy's fork of impacket]] :
...
Reference in [[https://pkiblog.knobloch.info/esc12-shell-access-to-adcs-ca-with-yubihsm|here]].
...
Use [[https://github.com/JonasBK/Powershell/blob/master/Check-ADCSESC13.ps1|Check-ADCSESC13.ps1]] to find OIDToGroupLink:
...
```
```
For more specific attack methods in various attack scenarios, please refer to the following: [[https://posts.specterops.io/adcs-esc14-abuse-technique-333a004dc2b9#aca0|adcs-esc14-abuse-technique]].
...
The following is referenced to [[(https://github.com/ly4k/Certipy/wiki/06-%E2%80%90-Privilege-Escalation#esc15-arbitrary-application-policy-injection-in-v1-templates-cve-2024-49019-ekuwu|this link]],Click to see more detailed usage methods.
...
The following is referenced to [[https://github.com/ly4k/Certipy/wiki/06-%E2%80%90-Privilege-Escalation#esc16-security-extension-disabled-on-ca-globally|this link]],Click to see more detailed usage methods.
...
- [[https://specterops.io/blog/2025/08/11/certify-2-0/|Certify 2.0 – SpecterOps Blog]]
