...
```
```
Upon acquiring the CA certificate and its private key in `.pfx` format, tools like [[https://github.com/GhostPack/ForgeCert|ForgeCert]] can be utilized to generate valid certificates:
...
[[account-persistence.md|account-persistence]] for more on explicit mappings.
...
> Post-2025 mapping considerations: placing a rogue CA in NTAuth only establishes trust in the issuing CA. To use leaf certificates for logon when DCs are in **Full Enforcement**, the leaf must either contain the SID security extension or there must be a strong explicit mapping on the target object (for example, Issuer+Serial in `altSecurityIdentities`). See [[account-persistence.md]].

## Malicious Misconfiguration - DPERSIST3

Opportunities for **persistence** through **security descriptor modifications of AD CS** components are plentiful. Modifications described in the "[[domain-escalation.md)" section can be maliciously implemented by an attacker with elevated access. This includes the addition of "control rights" (e.g., WriteOwner/WriteDACL/etc.|Domain Escalation]] to sensitive components such as:
...
Practical knobs attackers may set for long-term domain persistence (see [[domain-escalation.md]] for full details and detection):
