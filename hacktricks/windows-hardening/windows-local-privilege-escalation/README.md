...
This [[https://msrc.microsoft.com/update-guide/vulnerability|site]] is handy for searching out detailed information about Microsoft security vulnerabilities. This database has more than 4,700 security vulnerabilities, showing the **massive attack surface** that a Windows environment presents.

**On the system**

- _post/windows/gather/enum_patches_
- _post/multi/recon/local_exploit_suggester_
- [[https://github.com/rasta-mouse/Watson|_watson_]]
- [[https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite) _(Winpeas has watson embedded|_winpeas_]]_
...
In orther to exploit this vulnerabilities you can use tools like: [[https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus|Wsuxploit]]- These are MiTM weaponized exploits scripts to inject 'fake' updates into non-SSL WSUS traffic.

Read the research here:

[[files/CTX_WSUSpect_White_Paper (1).pdf]]

**WSUS CVE-2020-1013**

[[https://www.gosecure.net/blog/2020/09/08/wsus-attacks-part-2-cve-2020-1013-a-windows-10-local-privilege-escalation-1-day/|**Read the complete report here**]].\
Basically, this is the flaw that this bug exploits:

> If we have the power to modify our local user proxy, and Windows Updates uses the proxy configured in Internet Explorer’s settings, we therefore have the power to run [[https://github.com/GoSecure/pywsus|PyWSUS]] locally to intercept our own traffic and run code as an elevated user on our asset.
>
> Furthermore, since the WSUS service uses the current user’s settings, it will also use its certificate store. If we generate a self-signed certificate for the WSUS hostname and add this certificate into the current user’s certificate store, we will be able to intercept both HTTP and HTTPS WSUS traffic. WSUS uses no HSTS-like mechanisms to implement a trust-on-first-use type validation on the certificate. If the certificate presented is trusted by the user and has the correct hostname, it will be accepted by the service.

You can exploit this vulnerability using the tool [[https://github.com/GoSecure/wsuspicious) (once it's liberated|**WSUSpicious**]].
...
[[../stealing-credentials/credentials-protections.md#wdigest|**More info about WDigest in this page**]].
...
[[../stealing-credentials/credentials-protections.md#lsa-protection|**More info about LSA Protection here**]].
...
**Credential Guard** wasn introduced in **Windows 10**. Its purpose is to safeguard the credentials stored on a device against threats like pass-the-hash attacks.| [[../stealing-credentials/credentials-protections.md#credential-guard|**More info about Credentials Guard here.**]]
...
[[../stealing-credentials/credentials-protections.md#cached-credentials|**More info about Cached Credentials here**]].
...
**Learn more** about what is a **token** in this page: [[../authentication-credentials-uac-and-efs/index.html#access-tokens|**Windows Tokens**]].\
...
Check if you can **overwrite some binary running** or if you have write permissions of the binary folder to exploit possible [[dll-hijacking/README.md|DLL Hijacking attacks]]:
...
```
```
Always check for possible [[../../linux-hardening/privilege-escalation/electron-cef-chromium-debugger-abuse.md|**electron/cef/chromium debuggers** running, you could abuse it to escalate privileges]].
...
```
```
**Checking permissions of the folders of the processes binaries (**[[dll-hijacking/README.md|DLL Hijacking]]**)**
...
```
```
[[https://github.com/ankh2054/windows-pentest/raw/master/Privelege/accesschk-2003-xp.exe|You can download accesschk.exe for XP for here]]
...
**Check if you can modify the binary that is executed by a service** or if you have **write permissions on the folder** where the binary is located ([[dll-hijacking/README.md|DLL Hijacking]])**.**\
...
[[../basic-cmd-for-pentesters.md#firewall) **(list rules, create rules, turn off, turn off...|**Check this page for Firewall related commands**]]**

More[[../basic-cmd-for-pentesters.md#network| commands for network enumeration here]]
...
```
```
Note that mimikatz, lazagne, [[https://www.nirsoft.net/utils/credentials_file_view.html), [VaultPasswordView](https://www.nirsoft.net/utils/vault_password_view.html), or from [Empire Powershells module](https://github.com/EmpireProject/Empire/blob/master/data/module_source/credentials/dumpCredStore.ps1|credentialfileview]].
...
Encrypted user RSA keys, by using DPAPI, are stored in the `%APPDATA%\Microsoft\Protect\{SID}` directory, where `{SID}` represents the user's [[https://en.wikipedia.org/wiki/Security_Identifier|Security Identifier]]. **The DPAPI key, co-located with the master key that safeguards the user's private keys in the same file**, typically consists of 64 bytes of random data. (It's important to note that access to this directory is restricted, preventing listing its contents via the `dir` command in CMD, though it can be listed through PowerShell).
...
This code was extracted from [[https://github.com/PowerShellMafia/PowerSploit/blob/master/Privesc/PowerUp.ps1|**PowerUP**]]:
...
```
```
[[https://blog.ropnop.com/extracting-ssh-private-keys-from-windows-10-ssh-agent/|**Extract openssh keys from registry.**]]
...
- [[https://github.com/djhohnstein/SharpWeb|**SharpWeb**]]
- [[https://github.com/djhohnstein/SharpChromium|**SharpChromium**]]
- [[https://github.com/GhostPack/SharpDPAPI|**SharpDPAPI**]]
...
[[https://github.com/carlospolop/MSF-Credentials|**MSF-Credentials Plugin**]] **is a msf** plugin I have created this plugin to **automatically execute every metasploit POST module that searches for credentials** inside the victim.\
[[https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite|**Winpeas**]] automatically search for all the files containing passwords mentioned in this page.\
[[https://github.com/AlessandroZ/LaZagne|**Lazagne**]] is another great tool to extract password from a system.

The tool [[https://github.com/Arvanaghi/SessionGopher) search for **sessions**, **usernames** and **passwords** of several tools that save this data in clear text (PuTTY, WinSCP, FileZilla, SuperPuTTY, and RDP|**SessionGopher**]]
...
[[leaked-handle-exploitation.md|Read this example for more information about **how to detect and exploit this vulnerability**.]]\
[[http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/|Read this **other post for a more complete explanation on how to test and abuse more open handlers of processes and threads inherited with different levels of permissions (not only full access)**]].
...
When data is sent through a pipe by a **client**, the **server** that set up the pipe has the ability to **take on the identity** of the **client**, assuming it has the necessary **SeImpersonate** rights. Identifying a **privileged process** that communicates via a pipe you can mimic provides an opportunity to **gain higher privileges** by adopting the identity of that process once it interacts with the pipe you established. For instructions on executing such an attack, helpful guides can be found [[named-pipe-client-impersonation.md) and [**here**](#from-high-integrity-to-system|**here**]].

Also the following tool allows to **intercept a named pipe communication with a tool like burp:** [[https://github.com/gabriel-sztejnworcel/pipe-intercept) **and this tool allows to list and see all the pipes to find privescs** [**https://github.com/cyberark/PipeViewer**](https://github.com/cyberark/PipeViewer|**https://github.com/gabriel-sztejnworcel/pipe-intercept**]]
...
The technique described [[https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks) with a exploit code [**available here**](https://github.com/thezdi/PoC/tree/main/FilesystemEoPs|**in this blog post**]].
...
[[#alwaysinstallelevated|More information about the registry keys involved and how to install a _.msi_ package here.]]

### High + SeImpersonate privilege to System

**You can** [[seimpersonate-from-high-to-system.md|**find the code here**]]**.**
...
**You can find an** [[sedebug-+-seimpersonate-copy-token.md|**example of code executing the proposed technique here**]]**.**

### **Named Pipes**

This technique is used by meterpreter to escalate in `getsystem`. The technique consists on **creating a pipe and then create/abuse a service to write on that pipe**. Then, the **server** that created the pipe using the **`SeImpersonate`** privilege will be able to **impersonate the token** of the pipe client (the service) obtaining SYSTEM privileges.\
If you want to [[#named-pipe-client-impersonation|**learn more about name pipes you should read this**]].\
If you want to read an example of [[from-high-integrity-to-system-with-name-pipes.md|**how to go from high integrity to System using name pipes you should read this**]].

### Dll Hijacking

If you manages to **hijack a dll** being **loaded** by a **process** running as **SYSTEM** you will be able to execute arbitrary code with those permissions. Therefore Dll Hijacking is also useful to this kind of privilege escalation, and, moreover, if far **more easy to achieve from a high integrity process** as it will have **write permissions** on the folders used to load dlls.\
**You can** [[dll-hijacking/README.md|learn more about Dll hijacking here]]**.**
...
[[https://github.com/PowerShellMafia/PowerSploit) **-- Check for misconfigurations and sensitive files (**[**check here**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**|**PowerSploit-Privesc(PowerUP)**]]. Detected.**\
[[https://github.com/411Hall/JAWS) **-- Check for some possible misconfigurations and gather info (**[**check here**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**|**JAWS**]].**\
[[https://github.com/enjoiz/Privesc|**privesc** ]]**-- Check for misconfigurations**\
[[https://github.com/Arvanaghi/SessionGopher|**SessionGopher**]] **-- It extracts PuTTY, WinSCP, SuperPuTTY, FileZilla, and RDP saved session information. Use -Thorough in local.**\
[[https://github.com/peewpw/Invoke-WCMDump|**Invoke-WCMDump**]] **-- Extracts crendentials from Credential Manager. Detected.**\
[[https://github.com/dafthack/DomainPasswordSpray|**DomainPasswordSpray**]] **-- Spray gathered passwords across domain**\
[[https://github.com/Kevin-Robertson/Inveigh|**Inveigh**]] **-- Inveigh is a PowerShell ADIDNS/LLMNR/mDNS/NBNS spoofer and man-in-the-middle tool.**\
[[https://github.com/absolomb/WindowsEnum/blob/master/WindowsEnum.ps1|**WindowsEnum**]] **-- Basic privesc Windows enumeration**\
[[https://github.com/rasta-mouse/Sherlock) **\~\~**\~\~ -- Search for known privesc vulnerabilities (DEPRECATED for Watson|~~**Sherlock**~~]]\
[[https://github.com/A-mIn3/WINspect) -- Local checks **(Need Admin rights|~~**WINspect**~~]]**

**Exe**

[[https://github.com/rasta-mouse/Watson) -- Search for known privesc vulnerabilities (needs to be compiled using VisualStudio) ([**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/watson)|**Watson**]]\
[[https://github.com/GhostPack/Seatbelt) -- Enumerates the host searching for misconfigurations (more a gather info tool than privesc) (needs to be compiled) **(**[**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**|**SeatBelt**]]**\
[[https://github.com/AlessandroZ/LaZagne) **-- Extracts credentials from lots of softwares (precompiled exe in github|**LaZagne**]]**\
[[https://github.com/GhostPack/SharpUp|**SharpUP**]] **-- Port of PowerUp to C#**\
[[https://github.com/AlessandroZ/BeRoot) **\~\~**\~\~ -- Check for misconfiguration (executable precompiled in github|~~**Beroot**~~]]. Not recommended. It does not work well in Win10.\
[[https://github.com/pentestmonkey/windows-privesc-check) -- Check for possible misconfigurations (exe from python|~~**Windows-Privesc-Check**~~]]. Not recommended. It does not work well in Win10.

**Bat**

[[https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS)-- Tool created based in this post (it does not need accesschk to work properly but it can use it|**winPEASbat** ]].

**Local**

[[https://github.com/GDSSecurity/Windows-Exploit-Suggester) -- Reads the output of **systeminfo** and recommends working exploits (local python|**Windows-Exploit-Suggester**]]\
[[https://github.com/bitsadmin/wesng) -- Reads the output of **systeminfo** andrecommends working exploits (local python|**Windows Exploit Suggester Next Generation**]]

**Meterpreter**

_multi/recon/local_exploit_suggestor_

You have to compile the project using the correct version of .NET ([[https://rastamouse.me/2018/09/a-lesson-in-.net-framework-versions/)|see this]]. To see the installed version of .NET on the victim host you can do:
...
- [[https://research.checkpoint.com/2025/silver-fox-apt-vulnerable-drivers/|Check Point Research – Chasing the Silver Fox: Cat & Mouse in Kernel Shadows]]
