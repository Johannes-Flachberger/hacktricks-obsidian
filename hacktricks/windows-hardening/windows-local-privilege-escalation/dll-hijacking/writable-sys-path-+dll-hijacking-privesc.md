...
After finding this, I found this interesting blog post that also explains how to [[https://juggernaut-sec.com/dll-hijacking/#Windows_10_Phantom_DLL_Hijacking_-_WptsExtensionsdll|**abuse WptsExtensions.dll for privesc**]]. Which is what we **are going to do now**.
...
You can [[#creating-and-compiling-dlls|**try to use any of these examples**]]. You could run payloads such as: get a rev shell, add a user, execute a beacon...
...
> However, that user has the **`seImpersonate`** privilege, so you can use the[[../roguepotato-and-printspoofer.md| **potato suite to escalate privileges**]]. So, in this case a rev shell is a better option that trying to create a user.
